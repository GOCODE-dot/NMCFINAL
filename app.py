...
# ── Global 500 handler for API-style routes ───────────────────────────────────
# Every dashboard fetch() call does `await res.json()` and treats a parse
# failure as a generic "Network error — please try again." That's fine for
# a real connectivity problem, but it also fires whenever the server itself
# crashes with an *unhandled* exception, because Flask's default error page
# is HTML, not JSON — so a real server-side bug ends up looking like a
# network problem to the student/manager/admin. This handler makes sure any
# route under /student/, /manager/, /admin/, or /api/ always answers with
# JSON on a 500, so the toast shows the real error instead of a generic one.
@app.errorhandler(500)
def handle_internal_error(e):
    if request.path.startswith(('/api/', '/manager/', '/student/', '/admin/')) \
            or request.is_json or request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        original = getattr(e, 'original_exception', None)
        print('[NMMS] Unhandled 500 on', request.path, '-', repr(original or e))
        return jsonify({'ok': False, 'msg': 'Something went wrong on the server. Please try again.'}), 500
    return e
...
def student_rupantorpay_pay():
    ...
    try:
        ...
        # Anything that goes wrong here — the gateway call OR the DB insert —
        # is now caught in one place, so the frontend always gets back JSON
        # (a clear "Could not start checkout: ..." message) instead of a raw
        # 500 error page, which is what was showing up as a generic
        # "Network error — please try again" toast.
        try:
            conn.rollback()
        except Exception:
            pass
...
def student_securepaybd_pay():
    ...
    try:
        ...
        # Same fix as the RupantorPay route above: catch the gateway call
        # AND the DB insert in one block so a failure always comes back as
        # JSON with a real message, not a raw 500 page that breaks the
        # frontend's res.json() and shows as "Network error".
        try:
            conn.rollback()
        except Exception:
            pass
